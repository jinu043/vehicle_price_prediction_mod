...
with st.container():
    ...
    with col1:
        ...
        actual_price = data[(data["vehicle_name"]==df.loc[0,"vehicle_name"])&(data["model"]==df.loc[0,"model"])&
                      ...
                      (data["fuel_type"]==df.loc[0,"fuel_type"])&(data["mileage"]==df.loc[0,"mileage"])&(data["spec"]==df.loc[0,"spec"])][["vehicle_name", "model", "price"]].reset_index(drop=True)
